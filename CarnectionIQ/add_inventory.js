...
export default function (data) {
    ...
    let res = http.post(ENDPOINTS.ADD_VEHICLE, vehiclePayload, { headers });
    
    let success = check(res, {
        "Vehicle Added": (r) => r.status === 200 || r.status === 201
    });
    ...
    if (!success) {
        if (res.status === 502) {
            ...
            console.error(`Add Vehicle Failed: ${res.status} ${res.body.substring(0, 200)}`);
        }
    }
